...
const ParticleBackground = ({ isMobile }: { isMobile?: boolean }) => {
  const particles = useMemo(() => {
    return Array.from({ length: isMobile ? 5 : 15 }).map((_, i) => ({
      id: i,
      x: Math.random() * 100,
      y: Math.random() * 100,
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none opacity-20">
      {particles.map((p) => (
        <motion.div
          ...
          className="absolute bg-brand-purple rounded-full translate-z-0 will-change-[transform,opacity]"
        />
      ))}
    </div>
  );
        ...
        titleComponent={
          <div className="flex flex-col items-center">
            ...
            <h2 className="text-4xl md:text-7xl font-black mb-8 leading-tight uppercase tracking-tighter italic">
              {t.whyNiya.title1} <br /> <span className="text-white/30">{t.whyNiya.title2}</span>
            </h2>
          </div>
        }
      >
...
export const ContainerScroll = ({
  ...
  return (
    <div
      className="h-[40rem] md:h-[70rem] flex items-center justify-center relative p-4 md:p-20"
      ref={containerRef}
    >
      <div
  ...
  return (
    <motion.div
      style={{
        rotateX: rotate,
        scale,
        boxShadow: isMobile ? "0 10px 30px rgba(0,0,0,0.5)" : "0 0 #0000004d, 0 9px 20px #0000004a, 0 37px 37px #00000042, 0 84px 50px #00000026, 0 149px 60px #0000000a, 0 233px 65px #00000003",
      }}
      className="max-w-6xl -mt-12 mx-auto h-[30rem] md:h-[45rem] w-full border border-white/10 p-2 md:p-4 bg-white/5 backdrop-blur-sm md:backdrop-blur-xl rounded-[40px] shadow-2xl relative overflow-hidden translate-z-0 will-change-transform"
    >
       {/* Background subtle glow inside the card */}
       <div className="absolute top-0 right-0 w-64 h-64 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-brand-purple/20 to-transparent rounded-full translate-z-0" />
