...
export function ProjectShowcase() {
    ...
    useEffect(() => {
        // Sync items when language changes
        setItems(t.projects.items);
    }, [lang, t]);
    ...
    const handleScroll = () => {
        ...
        const scrollWidth = scrollRef.current.scrollWidth - scrollRef.current.clientHeight;
        const scrollLeft = scrollRef.current.scrollLeft;
        const index = Math.round((scrollLeft / (scrollWidth || 1)) * (items.length - 1));
        setActiveIndex(index);
    };

    // Keep selectedItem translated if it is open
            ...
            <div className="mb-16 md:mb-24 text-center">
                ...
                >
                  ...
                  <h2 className="text-4xl md:text-7xl font-black italic uppercase tracking-tighter">
                    {t.projects.title} <span className="text-white/30">{t.projects.works}</span>
                  </h2>
                </motion.div>
            </div>
                ...
                <div className="relative">
                    ...
                    >
                        {items.map((item) => (
                            <motion.div
                                key={item.id}
                                className="w-[70vw] sm:w-[50vw] shrink-0 snap-center relative aspect-[4/5] max-h-[60vh] rounded-[2rem] overflow-hidden glass border-white/5"
                            ...
                            >
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black via-black/20 to-transparent p-10 flex flex-col justify-end">
                                    ...
                                    <p className="text-white/40 text-[10px] font-bold tracking-widest uppercase mt-3 flex items-center gap-2">
                                        {t.projects.viewCaseStudy} <span className="text-brand-purple">→</span>
                                    </p>
                                </div>
                            </motion.div>
                        ))}
